...
var CompilerCache = function ()
{
    ...
    this.versions          = {}; // map resource type to compiler version
    this.workQueues        = {}; // map resource type to JobQueue instance
    this.waitCount         = 0;
...
{
    compiler.send({
        type : IPCMessageType.VERSION_QUERY,
        data : {}
    });
};
...
CompilerCache.prototype.handleQueueExecute = function (task)
{
    this.emit('started', this, {
        ...
        compilerName   : task.compiler.name
    });

    task.compiler.send({
        type : IPCMessageType.BUILD_REQUEST,
        data : {
...
CompilerCache.prototype.handleMonitorStart = function (monitor)
{
    ...
    this.versions[name]   = 0;
    this.workQueues[name] = queue;
...
{
    this.emit('error', this, monitor.name, error);
}
...
CompilerCache.prototype.handleMonitorMessage = function (monitor, message)
{
    ...
    switch (type)
    {
        case IPCMessageType.VERSION_DATA:
            {
                this.versions[name] = data.version || 1;
            }
            break;

        case IPCMessageType.BUILD_RESULT:
            {
                var queue = this.workQueues[name];
                var task  = queue.front();
                this.emit('complete', this, {
                    input           : task.input,
                    compilerName    : name,
                    compilerVersion : this.versions[name],
...
CompilerCache.prototype.handleMonitorExit = function (monitor)
{
    ...
    this.childProcessCount--;
    delete this.compilers[monitor.name];
    delete this.versions[monitor.name];
    delete this.workQueues[monitor.name];
...
CompilerCache.prototype.startup = function (processorRoot, config)
{
    ...
    // set the number of child processes we're waiting for.
    this.waitCount  = keys.length;
    ...
    keys.forEach(
        ...
        {
            var  monitorConfig = config[key];
            var  monitor       = self.compilers[key];
            if (!monitor)
            {
                // create a new monitor and start it to spawn the child process.
                monitorConfig.path  = Path.join(processorRoot, monitorConfig.path);
                monitor             = Monitor.createMonitor(monitorConfig);
                monitor.name        = key;
...
CompilerCache.prototype.shutdown = function ()
{
    ...
    Object.keys(this.compilers).forEach(
...
/// @return The Monitor instance representing the data compiler process, or
/// null if no data compiler is registered to handle the specified resource.
CompilerCache.prototype.findCompiler = function (resourceType, platformName)
{
    ...
    var dc       = this.compilers[name1]; // try platform-specific first
    if (dc) return dc;
    else    return this.compilers[name2]; // try the generic version
};
