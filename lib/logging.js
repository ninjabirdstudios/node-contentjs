...
/// Global variable for tracking all of the named Log instances for this
/// process. This table is populated by the loggerDefineLogInstance() function.
var LogTable   = {};

/// A table used to access string names of months by month index from a Date.
var Months     = [
    'Jan', 'Feb', 'Mar',
    'Apr', 'May', 'Jun',
    'Jul', 'Aug', 'Sep',
...
/// Pre-pends a leading zero to a number value less than 10.
...
/// Constructs a date and time value in based on either a user-specified date
/// and time, or the current date and time.
...
/// Constructor function for the Logger class.
...
/// Outputs a pre-formatted string to stdout.
...
/// Outputs a pre-formatted string to stderr.
...
/// Outputs a pre-formatted string to the current log file.
...
/// Examines the file size of the current log file and rolls the log over into
/// a new file necessary. This function is called repeatedly on an interval
/// configured in the Log.start() function.
...
/// Starts the log rolling process for filesystem logging.
...
/// Stops the log rolling process for filesystem logging and prevents any
/// output from being written to the log file(s).
/// @return The reference 'this'.
...
/// Prepends a date and timestamp value to a log message.
...
/// Outputs a log message to enabled log endpoints. The output is tagged
/// with '#notice'.
...
/// Outputs an informational message to enabled log endpoints. The output is
/// tagged with both '#info'.
/// @param format The printf-style format string. See the documentation at:
/// http://nodejs.org/docs/latest/api/util.html#util_util_format_format
/// @param varargs A variable-length argument list to be substituted into the
/// format string.
...
/// Outputs a debug message to enabled log endpoints. The output is tagged
/// with both '#debug'.
/// @param format The printf-style format string. See the documentation at:
/// http://nodejs.org/docs/latest/api/util.html#util_util_format_format
/// @param varargs A variable-length argument list to be substituted into the
...
/// Outputs a warning message to enabled log endpoints. The output is tagged
/// with '#warn'.
/// @param format The printf-style format string. See the documentation at:
/// http://nodejs.org/docs/latest/api/util.html#util_util_format_format
/// @param varargs A variable-length argument list to be substituted into the
...
/// Outputs an error message to enabled log endpoints. The output is tagged
/// with '#error'.
/// @param format The printf-style format string. See the documentation at:
/// http://nodejs.org/docs/latest/api/util.html#util_util_format_format
/// @param varargs A variable-length argument list to be substituted into the
...
/// Outputs a critical error message to enabled log endpoints. The output is
/// tagged with both '#error' and '#crit'.
/// @param format The printf-style format string. See the documentation at:
/// http://nodejs.org/docs/latest/api/util.html#util_util_format_format
/// @param varargs A variable-length argument list to be substituted into the
/// format string.
...
/// Outputs an alert message to enabled log endpoints. The output is tagged
/// with both '#error' and '#alert'.
/// @param format The printf-style format string. See the documentation at:
/// http://nodejs.org/docs/latest/api/util.html#util_util_format_format
/// @param varargs A variable-length argument list to be substituted into the
/// format string.
...
/// Outputs an emergency message to enabled log endpoints. The output is tagged
/// with both '#error' and '#emerg'.
/// @param format The printf-style format string. See the documentation at:
/// http://nodejs.org/docs/latest/api/util.html#util_util_format_format
/// @param varargs A variable-length argument list to be substituted into the
/// format string.
...
/// Outputs the contents of an object to enabled log endpoints.
...
/// Captures the current time and stores in in an array associated with a
/// specified label. Call the timeEnd() function with the same label to
...
/// Captures the time elapsed between this call and a prior call to the time()
/// function, and outputs the time (in milliseconds) to enabled log endpoints.
/// Output is tagged with '#profile'.
...
/// Generates a stack trace and outputs it to enabled log endpoints.
...
/// Outputs system statistics to the enabled log endpoints. Information
...
/// Outputs the current time, in standard syslog format, to the enabled log
/// endpoints. Output is tagged with '#time'.
/// @return The reference 'this'.
...
/// Create a new Log instance initialized with the specified configuration.
/// @param config Configuration options for the Log instance. See the function
/// logCreateConfiguration() for a template.
...
/// Define a shared log instance associated with a particular name. If the Log
/// does not exist, it is created. If the name is currently in use, the
...
/// Deletes an existing log instance associated with a particular name.
...
/// Deletes all existing named Log instances and resets the global Log table.
